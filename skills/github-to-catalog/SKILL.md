...
- `worker`（或 `pages`/`hybrid`）可附加 `assets` 字段（Worker with Assets / Pages 静态资源）：`"assets": { "source": { "kind": "raw", "url": ".../assets.zip" }, "binding"?: "ASSETS", "config"?: { "html_handling"?, "not_found_handling"? } }`。`source` 为必填，zip 内文件应在根目录（对应 wrangler 的 `directory`）；`binding` 缺省为 `ASSETS`，`config` 仅在需要自定义 SPA 回退/404 时填。
...
2. 判定类型并打包：
   - **Worker（需构建或归一成单文件）**：
     - 带 `package.json` 的构建型项目：`cd .tmp/<id> && npm install && npm run build`，取构建产物（如 `dist/_worker.js` / `dist/index.js`）复制为 `templates/<id>/worker.js`。
