...
| [MoeMail](https://github.com/beilunyang/moemail) | `moemail` | pages | 基于 Next.js + Cloudflare Pages 技术栈构建的可爱临时邮箱服务，支持多语言、多邮件渠道与附件，使用 D1 与 KV 存储。注意：本模板仅部署主站（Pages），邮件接收 Worker（email-receiver）与定时清理 Worker（cleanup）需另行部署，并需自行配置 Cloudflare Email Routing；D1 数据表需通过仓库的 drizzle 迁移初始化。 | 邮箱, pages, nextjs, d1 |
| [Sink](https://github.com/ccbikai/sink) | `sink` | pages | 100% 运行在 Cloudflare 上的极简、快速、安全的短链服务，内置分析面板与控制台，基于 Nuxt + Workers AI + KV + R2 构建。 | 短链, pages, ai, kv, r2 |
...
| [Cloudflare Docker Proxy](https://github.com/ciiiii/cloudflare-docker-proxy) | `cloudflare-docker-proxy` | worker | 运行在 Cloudflare Workers 上的 Docker 镜像代理，中转对 Docker 官方镜像仓库的请求（注意官方已对 Worker IP 限流，不推荐生产使用）。 | worker, docker, 代理 |
...
| [GitHubbox](https://github.com/dferber90/githubbox) | `githubbox` | worker | 将 GitHub 仓库快速在 CodeSandbox 打开的 Cloudflare Worker：把地址中的 github.com 改为你的 Worker 域名即可自动跳转到 CodeSandbox。支持仓库、分支、标签、commit 及文件路径。 | worker, 工具, github, 跳转 |
| [Cloudflare Temp Email](https://github.com/dreamhunter2333/cloudflare_temp_email) | `cloudflare-temp-email` | worker | 基于 Cloudflare Workers + D1 + Assets 的全功能临时邮箱服务，支持多语言、邮件附件、自动回复、Telegram Bot、用户管理与管理后台。前端已预构建随 Worker 一同部署，Rust WASM 邮件解析高性能。部署后需在 Cloudflare 邮件路由中配置将邮件转发到此 Worker。 | 邮箱, worker, d1, assets, 临时邮箱 |
| [Emaction Backend](https://github.com/emaction/emaction.backend) | `emaction-backend` | worker | GitHub 风格的 Reactions（点赞/表情）后端，基于 Cloudflare Workers + D1 实现。提供 GET /reactions 与 PATCH /reaction 接口，可私有化自托管互动计数数据。 | worker, d1, reactions, 点赞, 后端, api |
| [GitPush](https://github.com/fatwang2/gitpush) | `gitpush` | worker | 基于 Cloudflare Workers + Workflows + Workers AI + Email Routing 的 GitHub Release 追踪器，每日定时拉取关注的仓库 Release，用 Workers AI 总结更新内容并通过邮件推送。部署后需手动在 Cloudflare 后台补充两个绑定才能正常运行：1) Workflow 绑定 name=GITPUSH、class_name=GitPushWorkflow；2) send_email 绑定 name=SEND_EMAIL（目标地址为 EMAIL_TO_ADDRESS）。缺少这两个绑定时定时任务与发邮件会报错。 | worker, ai, workflow, 邮件, 监控 |
...
| [Text2img Cloudflare Workers](https://github.com/huarzone/text2img-cloudflare-workers) | `text2img-cloudflare-workers` | worker | 基于 Cloudflare Workers AI 的在线文生图服务，免费部署，调用 @cf/meta/stable-diffusion-xl-base-1.0 等文本生成图像模型，内置响应式 Web UI。前端 index.html 已随 Worker 打包为单文件，一键部署即可使用。 | worker, ai, 文生图, workers-ai |
| [Telegram Counter](https://github.com/iamshaynez/telegram-counter) | `telegram-counter` | worker | 使用 Cloudflare Worker + D1 + Workers AI 编写的 Telegram 机器人后端，可在 Telegram 中通过简单命令进行打卡、查询、设置目标与翻译。 | worker, d1, ai, telegram |
| [Gins Blog](https://github.com/ichimarugin728/Gins-Blog) | `gins-blog` | worker | 基于 Astro + Cloudflare 技术栈构建的多功能博客平台，内置评论、点赞、音乐播放、AI 对话与向量检索；使用 D1、R2、KV、Workers AI 等多种 Cloudflare 服务。注意：Catalog Schema 当前不支持 Vectorize 绑定，部署后需手动在 Worker 上补加 VECTOR_INDEX 绑定（index_name: gins-vector）才能使 AI 向量检索功能生效。 | 博客, astro, d1, r2, worker-ai, kv |
| [CF Image Hosting](https://github.com/ifyour/cf-image-hosting) | `cf-image-hosting` | pages | 基于 Cloudflare Pages 的免费无限图床，使用 Telegraph 存储，支持拖拽与粘贴上传图片/视频/GIF，单文件上限 5MB。基于 Hono + Vite 构建，已预编译为 Pages 部署包。 | pages, 图床, telegraph, hono |
...
| [丁丁快传 (DingDing)](https://github.com/iiop123/dingding) | `dingding` | worker | 基于 Cloudflare Workers 的文件传输工具「丁丁快传」，文件存储在 KV，支持拖拽/多文件上传、定时删除、二维码接收与文件口令传输。前端已打包为静态资源，随 Worker 通过 Worker Assets 一同部署。 | worker, 文件传输, kv |
| [Workers Image Hosting](https://github.com/iiop123/workers-image-hosting) | `iiop123-workers-image-hosting` | worker | 基于 Cloudflare Workers 数据存储于 KV 的图床，Material 风格，支持拖拽上传、图片预览与 /list.html 查询已上传照片。默认密码为 123，可通过环境变量 PASS 自定义。 | worker, 图床, kv |
| [SubPool Worker](https://github.com/illusionlie/subpool-worker) | `subpool-worker` | worker | 基于 Cloudflare Workers 的轻量订阅池服务，统一管理多组订阅来源、按客户端格式分发，含 Web 管理后台（Worker + KV + Assets）。 | worker, 订阅, kv, assets |
| [Edgebin](https://github.com/jiacai2050/edgebin) | `edgebin` | worker | 类似 httpbin 的 HTTP 测试服务，基于 Cloudflare Workers + Assets 边缘部署，支持请求/响应检视、重定向、Cookie、缓存、二维码、Markdown 互转等。 | worker, 工具, http |
| [CloudFlare Pages UrlShorten](https://github.com/jiaocz/CloudFlare-Pages-UrlShorten) | `cloudflare-pages-urlshorten` | pages | 基于 Cloudflare Pages Functions 的多功能 URL 短链工具，使用 KV 存储，通过 Pages Functions 实现路由与跳转。 | pages, 短链, kv |
...
| [UptimeFlare](https://github.com/lyc8503/UptimeFlare) | `uptimeflare` | hybrid | 基于 Cloudflare Workers + D1 的无服务器宕机监控与状态页，支持多地区探测、100+ 通知渠道、暗色状态页与自定义域名。 | 监控, 状态页, pages, d1, uptime |
...
| [Short (igengdu)](https://github.com/igengdu/short) | `short` | worker | 基于 Cloudflare Workers + D1 的 URL 缩短器，Fork 自 x-dr/short。支持自定义短链、访问日志记录与可选 BasicAuth 管理面板，前端使用 Alpine.js，随 Worker Assets 一同部署。 | 短链, worker, d1 |
| Siri Ultra | `siri-ultra` | worker | 基于 Cloudflare Workers 的 ChatGPT 风格 AI 聊天助手，支持多模型、流式回复与对话历史记忆。 |  |
| [GitHub 代理加速](https://github.com/crazypeace/gh-proxy) | `gh-proxy` | worker | 解决 GitHub 资源访问问题，支持 releases、archive、文件下载、clone、api.github.com、git.io、gitlab.com 等多种访问方式的代理服务，支持 GitHub 脚本无限嵌套调用。 | 代理, 加速, github, worker |
| [Url Shorten Worker (crazypeace)](https://github.com/crazypeace/Url-Shorten-Worker) | `url-shorten-worker-cf` | worker | 部署在 Cloudflare Workers 上的短链接服务，使用 KV 存储，支持自定义短链、访问统计、SPA/PWA 前台与多套主题。静态资源随 Worker Assets 本地托管，不依赖外部 GitHub Pages。 | 短链, worker, kv, assets |
| [Status Page](https://github.com/eidam/cf-workers-status-page) | `cf-workers-status-page` | worker | 基于 Cloudflare Workers + KV + CRON Triggers 的网站状态监控页面，使用 Flareact（React SSR）在边缘渲染，展示服务可用性与响应时间历史直方图，支持 Slack/Discord/Telegram 通知。每分钟自动检查配置的监控目标。注意：监控目标列表在源码 config.yaml 中配置，自定义需 fork 修改后重新构建。 | 监控, status-page, kv, cron, react |
| [Webhook Debugger](https://github.com/brancogao/webhook-debugger) | `webhook-debugger` | worker | 基于 Cloudflare Workers + D1 + Assets 的 Webhook 调试与捕获工具：实时接收任意 Webhook 请求，在仪表盘检视请求头、正文与查询参数，支持标签/备注管理与 GitHub 登录，前端随 Worker Assets 一同部署，并通过 CRON 每日自动清理过期数据。 | worker, webhook, 调试, d1, assets |
