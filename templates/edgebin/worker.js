...
var require_byte_data = __commonJS({
  "node_modules/qrcode/lib/core/byte-data.js"(exports, module) {
    ...
    ByteData.prototype.write = function(bitBuffer) {
      for (let i = 0, l3 = this.data.length; i < l3; i++) {
...
var O, E, m, xe, be, Re, C, Oe, j, se, ie, Te, F, we, Q, ye, Pe, v, U, Se, oe, $e, K, re, _e, Le, Me, ze, ae, Ae, D, W, le, Ee, ue, Ce, Ie, Be, pe, qe, ve, ce, De, He, Ze, Ge, Ne, je, Fe, q, Qe, he, de, Ue, X, Ke, N, We, I, M, Xe, ke, y, b, P, $, R, S, B, _, Dt, Ht, Zt, Gt, Nt, jt, Ft, Qt;
var init_marked_esm = __esm({
  "node_modules/marked/lib/marked.esm.js"() {
    ...
    X = { _backpedal: E, anyPunctuation: Ge, autolink: Ne, blockSkip: Be, br: ae, code: ze, del: E, emStrongLDelim: qe, emStrongRDelimAst: De, emStrongRDelimUnd: Ze, escape: Me, link: Qe, nolink: de, punctuation: Ee, reflink: he, reflinkSearch: Ue, tag: Fe, text: Ae, url: E };
    Ke = { ...X, link: h(/^!?\[(label)\]\((.*?)\)/).replace("label", q).getRegex(), reflink: h(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", q).getRegex() };
    N = { ...X, emStrongRDelimAst: He, emStrongLDelim: ve, url: h(/^((?:ftp|https?):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/, "i").replace("email", /[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(), _backpedal: /(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/, del: /^(~~?)(?=[^\s~])((?:\\[\s\S]|[^\\])*?(?:\\[\s\S]|[^\s~\\]))\1(?=[^~]|$)/, text: /^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|https?:\/\/|ftp:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/ };
...
var require_translator = __commonJS({
  "node_modules/node-html-markdown/dist/translator.js"(exports) {
    ...
    var isTranslatorConfig = (v2) => typeof v2 === "object";
...
var require_matcher = __commonJS({
  "node_modules/node-html-parser/dist/matcher.js"(exports) {
    ...
    function findOne(test, elems) {
      var elem = null;
      for (var i = 0, l3 = elems === null || elems === void 0 ? void 0 : elems.length; i < l3 && !elem; i++) {
        var el = elems[i];
        if (test(el)) {
          elem = el;
        } else {
    ...
    function findAll(test, nodes) {
      var result = [];
      for (var i = 0, j2 = nodes.length; i < j2; i++) {
        if (!isTag(nodes[i]))
          continue;
        if (test(nodes[i]))
          result.push(nodes[i]);
    ...
    var HTMLElement = (
      ...
      (function(_super) {
        ...
        HTMLElement2.prototype.closest = function(selector) {
          ...
          function findOne(test, elems) {
            var elem = null;
            for (var i = 0, l3 = elems.length; i < l3 && !elem; i++) {
              var el_1 = elems[i];
              if (test(el_1)) {
                elem = el_1;
              } else {
        ...
        Object.defineProperty(HTMLElement2.prototype, "rawAttributes", {
          ...
          get: function() {
            ...
            var attrs = {};
            if (this.rawAttrs) {
              var re2 = /([a-zA-Z()[\]#@$.?:][a-zA-Z0-9-_:()[\]#]*)(?:\s*=\s*((?:'[^']*')|(?:"[^"]*")|\S+))?/g;
              var match = void 0;
...
var require_utilities = __commonJS({
  "node_modules/node-html-markdown/dist/utilities.js"(exports) {
    ...
    var truthyStr = (v2, value) => v2 ? value !== void 0 ? value : String(v2) : "";
    ...
    function getChildNodes(node) {
      ...
      return res;
      function isNodeList(v2) {
        return v2 != null || typeof v2[Symbol.iterator] === "function";
      }
    }
      ...
      "code": ({ node, parent, options: { codeFence, codeBlockStyle }, visitor }) => {
        ...
        if (!isCodeBlock)
          return {
            ...
            postprocess: ({ content }) => {
              var _a2, _b2;
              const delimiter = "`" + (((_b2 = (_a2 = content.match(/`+/g)) === null || _a2 === void 0 ? void 0 : _a2.sort((a, b2) => b2.length - a.length)) === null || _b2 === void 0 ? void 0 : _b2[0]) || "");
      ...
      "table": ({ visitor }) => ({
        ...
        postprocess: ({ content, nodeMetadata, node }) => {
          ...
          rows.forEach((cols, rowNumber) => {
            ...
            res += "\n";
            if (rowNumber === 0)
              res += "|" + colWidth.map((w2) => " " + "-".repeat(w2) + " |").join("") + "\n";
          });
          return res;
        }
      }),
    ...
    var Visitor = class {
      ...
      visitNode(node, textOnly, metadata) {
        ...
        if ((0, nodes_1.isTextNode)(node)) {
          if (node.wholeText) {
            (_a = (_e2 = node).text) !== null && _a !== void 0 ? _a : _e2.text = node.wholeText;
...
function getDateString({
  ...
  switch (format) {
    ...
    case "timestamp":
      return Date.now();
    case "locale":
      return (/* @__PURE__ */ new Date()).toLocaleString(locale, { timeZone });
    case "utc":
...
async function sleep(seconds) {
  await new Promise(
    (resolve) => setTimeout(resolve, Math.min(seconds, 10) * 1e3)
  );
}
async function getTextInput(req, searchParams) {
...
// src/qrcode.js
async function handleQRCode(userOptions) {
  const options = {
    type: "svg",
...
// src/markdown.js
async function handleMarkdownToHtml(req, searchParams) {
  const markdown = await getTextInput(req, searchParams);
  const module = await Promise.resolve().then(() => (init_marked_esm(), marked_esm_exports));
  return new Response(module.marked.parse(markdown), {
    headers: { "Content-Type": "text/html; charset=utf-8" }
  });
}
async function handleHtmlToMarkdown(req, searchParams) {
  const html = await getTextInput(req, searchParams);
  const module = await Promise.resolve().then(() => __toESM(require_dist2(), 1));
...
function handleSession(server, timeZone) {
  ...
  server.addEventListener("message", ({ data }) => {
    switch (data) {
      case "close":
        return server.close(1e3, "Normal Closure");
      case "date":
...
var websocketHandler = async (request) => {
...
// src/mix.js
async function handleMix(searchParams, req) {
  let status = searchParams.has("s") ? stringToNumber(searchParams.get("s")) : 200;
  ...
  searchParams.getAll("h").forEach((h2) => {
    const idx = h2.indexOf(":");
    if (idx > 0) {
      const [key, value] = [h2.slice(0, idx), h2.slice(idx + 1)];
...
// src/webhook.js
async function handleWebhook(searchParams, req) {
...
async function handlePageMeta(req, searchParams) {
  const url = searchParams.get("url");
  if (!url) {
  ...
  await new HTMLRewriter().on("title", titleExtractor).on("meta", metaExtractor).transform(response).arrayBuffer();
  return new Response(
    JSON.stringify(
      {
        url: response.url,
...
async function handle(req, env, ctx) {
  ...
  switch (parts[0]) {
    ...
    case "uuid": {
      const uuids = [];
      const count = parts.length > 1 ? Math.min(stringToNumber(parts[1]), 100) : 1;
      for (let i = 0; i < count; i++) {
        uuids.push(crypto.randomUUID());
      }
      return count === 1 ? Response.json({ uuid: uuids[0] }) : Response.json({ uuids });
    }
    case "date": {
      return Response.json({
        date: getDateString({
...
function handleCookies(parts, searchParams, req) {
  ...
  switch (action) {
    ...
    case "delete": {
      const headers = new Headers({ Location: "/cookies" });
      for (const [key, _2] of searchParams) {
        headers.append(
          "Set-Cookie",
...
function handleCache(parts, searchParams, req) {
  if (parts.length === 1) {
    if (req.headers.get("If-None-Match") || req.headers.get("If-Modified-Since")) {
  ...
  const headers = {
    ...
    Expires: new Date(Date.now() + seconds * 1e3).toUTCString(),
    "Last-Modified": (/* @__PURE__ */ new Date()).toUTCString(),
    ETag: `"${seconds}"`
  };
  return handleAnything(searchParams, req, { respHeaders: headers });
}
async function handleAnything(searchParams, req, { respHeaders = {}, extra = {}, status = 200 } = {}) {
