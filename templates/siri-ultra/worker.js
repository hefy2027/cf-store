...
// src/history.ts
var ChatHistory = class _ChatHistory {
  constructor(kv) {
    this.kv = kv;
  }
  static getInstance(kv) {
    if (!_ChatHistory.instance) {
      _ChatHistory.instance = new _ChatHistory(kv);
    }
    return _ChatHistory.instance;
  }
  async add(chat_id, message) {
...
var BASE_URL = "https://api.search1api.com/search";
var searchWeb = async (query, apiKey, maxResults = 10, crawlResults = 0) => {
  const url = BASE_URL;
  const options = {
    method: "POST",
    headers: {
...
var functions = [search];
var handle = async (name, args, req) => {
...
// src/chat.ts
function removeDuplicateBraces(jsonString) {
  const match2 = jsonString.match(/^{\s*"({.*})"\s*}$/s);
  if (match2 && match2[1]) {
...
var handle2 = async (req) => {
  ...
  while (true) {
    ...
    const ask = await openai.client.chat.completions.create({
      ...
      ...req.request.config?.search1api_key && req.request.config.search1api_key !== "your search1api_key" ? { tools: FunctionHandler.functions } : {}
    });
