...
// KV 绑定（由部署端注入；模块格式下在 export default.fetch 中赋值）
let LINKS;

// 静态资源（index/result 页面）由 Worker Assets 本地托管，不再依赖外部 GitHub Pages
let index_html = config.theme ? `/${config.theme}/index.html` : "/index.html"
let result_html = config.theme ? `/${config.theme}/result.html` : "/result.html"
...
async function handleRequest(request, env) {
  ...
  if (path == password_value) {
    let index = await env.ASSETS.fetch(new URL(index_html, request.url))
    index = await index.text()
    index = index.replace(/__PASSWORD__/gm, password_value)
    // 注入 R2 配置 (环境变量 / Secret)
    index = index.replace(/__R2_ACCOUNT_ID__/gm, (env.R2_ACCOUNT_ID !== undefined) ? env.R2_ACCOUNT_ID : '')
    index = index.replace(/__R2_ACCESS_KEY_ID__/gm, (env.R2_ACCESS_KEY_ID !== undefined) ? env.R2_ACCESS_KEY_ID : '')
    index = index.replace(/__R2_SECRET_ACCESS_KEY__/gm, (env.R2_SECRET_ACCESS_KEY !== undefined) ? env.R2_SECRET_ACCESS_KEY : '')
    index = index.replace(/__R2_BUCKET_NAME__/gm, (env.R2_BUCKET_NAME !== undefined) ? env.R2_BUCKET_NAME : '')
    index = index.replace(/__R2_PUBLIC_URL__/gm, (env.R2_PUBLIC_URL !== undefined) ? env.R2_PUBLIC_URL : '')
    // 将页面中引用作者 GitHub Pages 的静态资源改写为本 Worker 自身域名，实现自托管
    index = index.replace(/https:\/\/crazypeace\.github\.io\/Url-Shorten-Worker\//g, new URL(request.url).origin + "/")
    if (!config.load_kv) {
  ...
  if (config.result_page) {
    let result_page_html = await env.ASSETS.fetch(new URL(result_html, request.url))
    ...
    // 将页面中引用作者 GitHub Pages 的静态资源改写为本 Worker 自身域名，实现自托管
    result_page_html_text = result_page_html_text.replace(/https:\/\/crazypeace\.github\.io\/Url-Shorten-Worker\//g, new URL(request.url).origin + "/")
