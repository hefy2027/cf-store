 ...
 *
 *  本文件为 CF Store 模板版本：
 *  - 将 github_base 改为从绑定变量 GITHUB_BASE 读取，缺省回退到作者仓库；
 *  - 部署后在 cf-manager 中设置 GITHUB_BASE 为你自己的
 *    `owner/repo`（其中 posts/ 与 list.json 存放文章）即可。
 *
 */
...
// 文章来源仓库：默认回退到作者仓库，可通过绑定变量 GITHUB_BASE 覆盖
function getGithubBase(env) {
  if (env && env.GITHUB_BASE && env.GITHUB_BASE.trim() !== "") {
    return env.GITHUB_BASE.trim();
  }
  return "kasuganosoras/cloudflare-worker-blog";
}
...
async function bloghandle(request, env) {
  var $_GET = getRequestParams(request.url);
  var github_base = getGithubBase(env);
  var data = header;
  if($_GET['p'] == undefined) {
 ...
 * @param {Object} env binding variables
 */
async function handleRequest(request, env) {
  var resp = new Response(await bloghandle(request, env), {"Content-type": "text/html;charset=UTF-8", status: 200});
